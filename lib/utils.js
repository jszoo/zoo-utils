...
var utils = {
    ...
    arg2arr: function() {
        ...
        return function(args, start, end) {
            return slice.call(args, start || 0, end);
        };
    }(),
...
utils.dictionary = function(caseInsensitive) {
    this._data = {};
    if (caseInsensitive === true) {
        this.fmk = function(k) {
            return utils.formalStr(k);
        };
    }
};
utils.dictionary.prototype = {
    constructor: utils.dictionary, _data: null,
    fmk: function(k) { return k; },
    all: function() { return this._data; },
    get: function(key) { return this._data[this.fmk(key)]; },
    set: function(key, val) { this._data[this.fmk(key)] = val; },
    remove: function(key) { return delete this._data[this.fmk(key)]; },
    exists: function(key) { return (this.fmk(key) in this._data); },
    count: function() { return utils.propCount(this.all()); },
    clear: function() { this._data = {}; }
};

utils.Error = function() { this.message = utils.format.apply(utils, arguments); };
utils.Error.prototype = new Error();
utils.Error.prototype.constructor = utils.Error;
